 ...
 *
 * Implements the common system as resource for Smarty which does extend a chain of template files templates
 *
 * @package Smarty
...
class Smarty_Internal_Resource_Extends
{
    ...
    // classes used for compiling Smarty templates from common resource
     ...
     *
     * @param object $_template template object
     * @return string filepath to template source common
     */
     ...
     *
     * @param object $_template template object
     * @return integer timestamp of template source common
     */
    ...
    /**
     * Read template source from common
     *
     * @param object $_template template object
     * @return string content of template source common
     */
    public function getTemplateSource($_template)
    {
        ...
        foreach ($_files as $_file => $_filepath) {
            if ($_filepath === false) {
                throw new SmartyException("Unable to load template 'common : {$_file}'");
            }
            // read template common
            if ($_filepath != $_first) {
                $_template->properties['file_dependency'][sha1($_filepath)] = array($_filepath, filemtime($_filepath), 'common');
            }
            ...
            if ($_filepath != $_last) {
                if (preg_match_all("!({$this->_ldl}block\s(.+?){$this->_rdl})!", $_content, $_open) !=
                    ...
                    throw new SmartyException("unmatched {block} {/block} pairs in common '$_filepath'");
                }
