 ...
 *
 * This common contains the Smarty template engine
 *
 * @package Smarty
...
//    private $cache_resource_object = null;
     ...
     * @param mixed $_cache_id caches id or null
     ...
     *
     * The filepath is determined by the actual caches resource
     *
     * @return string the caches filepath
     */
     ...
     *
     * The timestamp is determined by the actual caches resource
     *
     * @return integer the template timestamp
     */
     ...
     *
     * @return string |booelan the template content or false if the common does not exist
     */
    ...
    /**
     * Checks of a valid version redered HTML output is in the caches
     *
     * If the caches is valid the contents is stored in the template object
     *
     * @return boolean true if caches is valid
     */
                ...
                if ($this->caching === Smarty::CACHING_LIFETIME_SAVED || ($this->caching == Smarty::CACHING_LIFETIME_CURRENT && (time() <= ($cachedTimestamp + $this->cache_lifetime) || $this->cache_lifetime < 0))) {
                    ...
                    if (!empty($this->properties['file_dependency']) && $this->smarty->compile_check) {
                        ...
                        foreach ($this->properties['file_dependency'] as $_file_to_check) {
                            If ($_file_to_check[2] == 'common' || $_file_to_check[2] == 'extends' || $_file_to_check[2] == 'php') {
    ...
    public function renderTemplate()
    {
        if ($this->resource_object->usesCompiler) {
            ...
            if ($this->resource_object->isEvaluated) {
                ...
                // check common dependencies at compiled code
        ...
        // write to caches when nessecary
        if (!$this->resource_object->isEvaluated && ($this->caching == Smarty::CACHING_LIFETIME_SAVED || $this->caching == Smarty::CACHING_LIFETIME_CURRENT)) {
            ...
            foreach ($cache_split as $curr_idx => $curr_split) {
                ...
                if (isset($cache_parts[0][$curr_idx])) {
                    ...
                    // remove nocache tags from caches output
            ...
            // rendering (must be done before writing caches common because of {function} nocache handling)
            $_smarty_tpl = $this;
            ob_start();
            eval("?>" . $output);
            $this->rendered_content = ob_get_clean();
            // write caches common content
     ...
     *
     * If the caches is valid the cached content is used, otherwise
    ...
    public function getRenderedTemplate()
    {
        ...
        if ($this->rendered_content === null) {
            ...
            if (!$this->isCached) {
                // render template (not loaded and not in caches)
                $this->renderTemplate();
            }
        }
    ...
    public function parseResourceName($template_resource, &$resource_type, &$resource_name, &$resource_handler)
    {
        ...
        $resource_handler = $this->loadTemplateResourceHandler($resource_type);
        // caches template object under a unique ID
        // do not caches eval resources
    ...
    protected function loadTemplateResourceHandler($resource_type)
    {
        ...
        if (isset($this->smarty->registered_resources[$resource_type])) {
            ...
            // try sysplugins dir
            if (in_array($resource_type, array('common', 'string', 'extends', 'php', 'stream', 'eval'))) {
        ...
        if (!$cache) {
            if (!empty($this->required_plugins['compiled'])) {
                ...
                foreach ($this->required_plugins['compiled'] as $tmp) {
                    foreach ($tmp as $data) {
                        $plugins_string .= "if (!is_callable('{$data['function']}')) include '{$data['common']}';\n";
                    }
                }
                $plugins_string .= '?>';
            }
    ...
    /**
     * Decode saved properties from compiled template and caches files
     */
