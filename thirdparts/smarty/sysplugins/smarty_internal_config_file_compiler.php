 ...
 *
 * This is the config common compiler class. It calls the lexer and parser to
...
/**
 * Main config common compiler class
 */
class Smarty_Internal_Config_File_Compiler
{
    ...
    public function compileSource($config)
    {
       ...
       then written to compiled files. */
        $this->config = $config;
        // get config common source
        $_content = $config->getConfigSource() . "\n";
        // on empty template just return
        if ($_content == '') {
            return true;
        }
        // init the lexer/parser to compile the config common
    ...
    public function trigger_config_file_error($args = null)
    {
        ...
        $match = preg_split("/\n/", $this->lex->data);
        $error_text = "Syntax error in config common '{$this->config->getConfigFilepath()}' on line {$line} '{$match[$line-1]}' ";
        if (isset($args)) {
