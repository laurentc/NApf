...
{
    // caching mode to create nocache code but no caches common
    const CACHING_NOCACHE_CODE = 9999;
    // attribute definitions
    public $required_attributes = array('common');
    public $shorttag_order = array('common');
    ...
    public function compile($args, $compiler)
    {
        ...
        if ($compiler->smarty->merge_compiled_includes || $_attr['inline'] === true) {
            ...
            if (!$compiler->has_variable_string && (substr_count($include_file, '"') == 2 or substr_count($include_file, "'") == 2) and substr_count($include_file, '(') == 0) {
                ...
                if ($this->compiler->template->template_resource != $tmp) {
                    ...
                    // suppress writing of compiled common
                    $tpl->write_compiled_code = false;
                    if ($this->compiler->template->caching) {
                        // needs code for cached page but no caches common
        ...
        * it will not be included into the common caches common and treated like
        * a nocache section
        */
        ...
        unset($_attr['common'], $_attr['assign'], $_attr['cache_id'], $_attr['compile_id'], $_attr['cache_lifetime'], $_attr['nocache'], $_attr['caching'], $_attr['scope'], $_attr['inline']);
