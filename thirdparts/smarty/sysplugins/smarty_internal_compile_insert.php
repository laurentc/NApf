...
class Smarty_Internal_Compile_Insert extends Smarty_Internal_CompileBase {
    ...
    public function compile($args, $compiler)
    {
        ...
        if (isset($_attr['script'])) {
            ...
            if ($_filepath == false) {
                $this->compiler->trigger_template_error("{insert} missing script common '{$_script}'", $this->compiler->lex->taglineno);
            } 
            // code for script common loading
            $_output .= "require_once '{$_filepath}' ;";
            require_once $_filepath;
            if (!is_callable($_function)) {
                $this->compiler->trigger_template_error(" {insert} function '{$_function}' is not callable in script common '{$_script}'", $this->compiler->lex->taglineno);
            } 
        } else {
            $_filepath = 'null';
